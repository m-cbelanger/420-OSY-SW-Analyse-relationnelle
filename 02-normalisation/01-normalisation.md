...
La facture possède d’un à plusieurs items, les items apparaissent dans 0 à plusieurs factures... On a une liaison * - *, donc la table de liaison est nécessaire! facture_item.
...
Cela signifie que la modification d’une colonne non-clé pourrait entraîner le changement d’une autre colonne non-clé. C'est aussi à cette étape qu'on va extraire des tables **lookup** (table servant de liste de valeurs fixes, qui ne changeront jamais ou rarement).
...
Entre factures et privilèges, on analyse qu'une facture a un seul niveau de privilège, mais que les niveaux de privilèges apparaissent dans plusieurs factures. Une clé de l'enfant vers le parent sera suffisante! relation 1 - *. Les tables normalisées suivent donc le schéma suivant:
...
- un utilisateur et ses informations (chauffeur ou covoitureur)
- une date de départ
- une heure de départ
...
D'autres champs pourraient être ajoutés comme l'heure de départ, le lieu exact de départ (adresse, spécification), l'ajout de commentaires, de détails sur la voiture, le "rating" de l'utilisateur, le rôle de l'utilisateur, le montant payé au chauffeur, etc. Pour l'exemple, on gardera seulement les infos ci-haut.
...
- pas de colonne qui se répète pour énumérer des infos de même type
...
Note: il n'y a pas de moment parfait pour extraire les lookups, on choisit souvent de le faire à la fin puisque c'est un détail qui n'a pas d'incidence sur la logique métier. On va donc en profiter ici pour faire une lookup pour les champs: 
